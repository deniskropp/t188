    ...
    <div className="w-full">
        ...
        {phases.map((p, idx) => {
          ...
          return (
            <motion.div 
              ...
              className={`relative flex items-center gap-4 p-3 rounded-lg transition-all duration-500 border ${
                isActive ? 'bg-meta-accent/10 border-meta-accent/30 shadow-lg shadow-meta-accent/5' : 
                'bg-meta-surface-hover border-meta-border opacity-50'
              } ${!isActive && !isDone ? 'opacity-20 grayscale' : ''}`}
            >
              <div className={`p-2 rounded-lg ${isActive ? 'bg-meta-accent text-white shadow-lg shadow-meta-accent/40' : 'bg-meta-surface text-meta-muted'}`}>
                <Icon size={16} />
              </div>
              <div className="flex-1 min-w-0">
                <div className={`text-[11px] font-bold ${isActive ? 'text-meta-main' : 'text-meta-muted'}`}>{p.label}</div>
                <div className="text-[9px] text-meta-muted font-medium truncate">{p.desc}</div>
              </div>
      ...
      {phase !== 'idle' && status && (
        <motion.div 
          ...
          className="mt-4 p-3 rounded-xl bg-meta-accent/5 border border-meta-accent/10 relative overflow-hidden"
        >
